...
if __name__ == "__main__":
    ...
    if arg == "updatets":
        ...
        cmd = PYLUPDATE + ' `find ./ -name "*.py"` `find ./ -name "*.ui"` -ts ' + os.path.join(transpath,MODULENAME+".ts")
        os .system(cmd)
        print("Updated",os.path.join(transpath,MODULENAME+".ts"))

    elif arg == "build":

        print("Building (warning, this can be invoked only once per 30 minutes)... ",end="")
        c = pycurl.Curl()
        ...
        c = pycurl.Curl()
        fields = [('files['+MODULENAME+'.ts]', (c.FORM_FILE, os.path.join(transpath,MODULENAME+".ts")))]
