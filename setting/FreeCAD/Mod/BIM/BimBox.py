...
class BIM_Box:
    ...
    def PointCallback(self,point,snapinfo):
        ...
        if len(self.points) == 0:
            ...
            FreeCADGui.Snapper.getPoint(last=point,callback=self.PointCallback,movecallback=self.MoveCallback,extradlg=self.taskbox())
            ...
            if hasattr(FreeCAD,"DraftWorkingPlane"):
                ...
                FreeCAD.DraftWorkingPlane.u = (self.cubetracker[0].p4().sub(self.cubetracker[0].p3())).normalize()
                FreeCAD.DraftWorkingPlane.v = FreeCAD.Vector(self.normal).normalize()
                FreeCAD.DraftWorkingPlane.axis = (self.cubetracker[0].p2().sub(self.cubetracker[0].p3())).normalize()
                FreeCADGui.Snapper.setGrid()
            FreeCADGui.Snapper.getPoint(last=self.cubetracker[0].p3(),callback=self.PointCallback,movecallback=self.MoveCallback,extradlg=self.taskbox())
        elif len(self.points) == 3:
    ...
    def taskbox(self):
        ...
        QtCore.QObject.connect(self.Length,QtCore.SIGNAL("returnPressed()"),self.setLengthUI)
        QtCore.QObject.connect(self.Width,QtCore.SIGNAL("returnPressed()"),self.setWidthUI)
        QtCore.QObject.connect(self.Height,QtCore.SIGNAL("returnPressed()"),self.setHeightUI)
    ...
    def setLengthUI(self):
        ...
        if (len(self.points) == 1) and self.currentpoint and self.LengthValue:
            ...
            self.cubetracker[0].p1(self.points[0])
            self.cubetracker[0].on()
            FreeCADGui.Snapper.getPoint(last=p2,callback=self.PointCallback,movecallback=self.MoveCallback,extradlg=self.taskbox())
